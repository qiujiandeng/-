...
class FtpServer(object):
    ...
    #上传文件
    def do_put(self):
        self.c.send("mima".encode())
        mi_ma = self.c.recv(128).decode()
        if mi_ma == '123456':
            self.c.send("OK".encode())
            file_name = self.c.recv(258).decode()
            aaaaa = file_name.strip().split("/")[-1]
            dd_file = FILE_PATH + aaaaa
            print(dd_file) 
            print('aaaaa = ',file_name.strip().split("/")[-1])
            
            fd = open(FILE_PATH + file_name.strip().split("/")[-1],'wb')
            while True:
